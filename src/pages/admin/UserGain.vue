...
//添加用户事件
const addUserPost = () => {
  http("post", "/admin/Api/User/register", store.userInfo).then(
...
function initTableData() {
  ...
  renderData(
    `/admin/Api/User/getUserList?limit=${toolInfo.limit}&page=${toolInfo.page}&search=${toolInfo.search}`
  );
}
...
<template>
  <div class="tableMain">
    ...
    <el-table
      ...
      stripe
      :size="store.isMobile ? 'default' : 'large'"
          ...
          <div class="expand">
            <el-descriptions border>
              ...
              <el-descriptions-item label="地址">
                <ElInput v-model="soonReviseUserData[props.$index].address"> </ElInput>
              </el-descriptions-item>
              <el-descriptions-item label="id">
                {{ soonReviseUserData[props.$index].id }}
              </el-descriptions-item>
              <el-descriptions-item label="操作">
                <ElButton @click="submitEvent(props.$index)" size="small"
                  >确认修改</ElButton
                >
                <ElButton
                  @click="resetEvent(props.row, props.$index)"
        ...
        <template #default="scope">
          <div class="operations">
            ...
            <el-button
              ...
              size="small"
              type="danger"
              @click="devastateUser(scope.row.id)"
              >删除</el-button
            >
          </div>
        </template>
